...
(define (eval-under-env e env)
  (letrec ([env-with-close (lambda (str val)
                           (cond ([(null? env) (cons (cons str val) null)]
                                  ...
                                  [#t (cons (car env) (env-with-close (cdr env) str))])))]
         [handle-fun (lambda (e) (if (fun-nameopt e)
                                     (closure (env-with-close (fun-nameopt e) (closure env e)) e)
                                     (closure env e)))]
         ...
         [handle-call (lambda (e) (letrec ([funexp (eval-under-env (call-funexp e))]
                                           [actual (eval-under-env (call-actual e))]
                                    ...
                                    (if (not closure? funexp)
                                        (error "not a closure")
                                        (eval-func (fun-body funexp)))))])  (cond ([(int? e) e]
                                                                                    [(closure? e) e]
                                                                                    [(fun? e) (handle-fun e)]  
                                                                                    [(var? e) (handle-var e)]
                                                                                    [(add? e) (handle-add e)]
