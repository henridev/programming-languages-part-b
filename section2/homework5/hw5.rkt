  ...
  (letrec ([create-new-env (lambda (variable-name value)
           ...
           [handle-fst(lambda (e) (let([v1 (eval-under-env (fst-e e) env)])
                                      (if (apair? v1)
                                          (apair-e1 v1)
                                          (error "enter a pair"))))]
           [handle-snd(lambda (e) (let([v1 (eval-under-env (snd-e e) env)])
                                      (if (apair? v1)
                                          (apair-e2 v1)
                                          (error "enter a pair"))))]
           ...
           [handle-is-greater (lambda (e) (let ([v1 (eval-under-env (ifgreater-e1 e) env)]
           ...
           [handle-call (lambda (e) (letrec ([a-closure (eval-under-env (call-funexp e) env)]
                                      ...
                                      (if (closure? a-closure)
                                          (let* ([fun-name (fun-nameopt (closure-fun a-closure))]
                                            ...
                                            (eval-under-env closure-fun-body  new-env))
                                          (error "not a closure")
                                          )))]
...
(define mupl-mapAddN
  (fun #f "my-int"
       (fun "mep-rec" "list"
            (mlet "map" mupl-map
                  (call (call (var "map") (fun #f "x" (add (var "my-int") (var "x")))) (var "list"))))))
